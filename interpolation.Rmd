...
# Representation of sites parameters estimated 
## Sites random effects 

```{r representation-alphas}
library(rspatial)

params_sites <- read.csv("data/params_sites.csv")
...
MADA <- readRDS("data/MADA.rds")
MADA <- spTransform(MADA, TA)
par(mfrow=c(1,1))
# define groups for mapping
cuts <- c(-1,-0.75,-0.5,-0.25,0,0.25,0.5,0.75,1)
col <- c('red3','red','orange','yellow', 'light green', 'green3', 'forest green','dark green')
pols <- list("sp.polygons", MADA, fill = "lightgray")
spplot(alpha_sp, 'alpha', cuts=cuts, col.regions=col, sp.layout=pols, pch=20, cex=1.5, key.space
       ="right", main ="Sites random effects estimated")
```
...
# define borderss 
pols <- list("sp.polygons", MADA, fill = "lightgray")
spplot(W1_sp, 'W1', cuts=cuts, col.regions=col, sp.layout=pols, pch=20, cex=1.5, key.space
       ="right", main ="W1 estimated")
```
### W2
```{r representation-W2}
...
col <- c('red3','red','orange','yellow', 'light green', 'green3', 'forest green','dark green')
pols <- list("sp.polygons", MADA, fill = "lightgray")
spplot(W2_sp, 'W2', cuts=cuts, col.regions=col, sp.layout=pols, pch=20, cex=1.5, key.space
       ="right", main ="W2 estimated")
```

# Interpolation of sites parameters
## Comparison of three methods on sites random effect
### Ordinary kriging 
```{r OK_alpha, cache = TRUE }
...
names(ok_alpha) <- c('alphas predicted', 'variance')
plot(ok_alpha)
